...
parser = argparse.ArgumentParser(
    description='Script for analysing results of several experiments')
parser.add_argument('--experiments_dir', type=str, default='logs',
                    help='Directory containing the results of the experiments')
parser.add_argument('--sort_by', type=str,
                    default='max_test_accuracy', help='Parameter to sort by')
parser.add_argument('--save_to', type=str,
                    help='If set saves to specified path. Type will be inferred from the extension. Available extensions (xlsx, csv)')


def check_line_length(el_iterable):
    for i in range(len(el_iterable)):
        e = str(el_iterable[i])
        if len(e) > 50:
            if isinstance(el_iterable[i][0], list):
                el_iterable[i] = ['\n'.join(el_iterable[i][0])]
            if isinstance(el_iterable[i][0], dict):
                el_iterable[i] = [yaml.dump(el_iterable[i][0], indent=1)]
...
if not param_files:
    logging.error(
        f"Provided directory {path}, does not contain any params file")
    exit(0)

common_path = os.path.commonpath(param_files)
all_same_names = len(np.unique([os.path.basename(x)
                     for x in param_files])) == 1
moving_avg_window = 3
for d in tqdm(param_files):
    with open(d, 'r') as f:
    ...
    for file in os.listdir(containing_dir):
        if file.startswith('events.out.tfevents'):
            ...
            min_losses = [round(min(np.convolve(x, np.ones(
                moving_avg_window)/moving_avg_window, mode='valid')), 2) for x in test_losses]
            max_accuracies = [round(max(np.convolve(x, np.ones(
                moving_avg_window)/moving_avg_window, mode='valid')), 2) for x in test_accuracies]
...
for x in all_params:
    all_params[x] = dict(
        sorted(all_params[x].items(), key=operator.itemgetter(0)))
...
single_val_rows = [[x] for x in np.array(
    list(list(all_params.values())[0].values()), dtype=object)[single_val_cols]]
single_val_params = list(np.array(list(list(all_params.values())[
                         0].keys()), dtype=object)[single_val_cols])
check_line_length(single_val_rows)
single_val_df = pd.DataFrame(
    single_val_rows, index=single_val_params, columns=['Common Values'])

print(tabulate.tabulate(single_val_df, headers=[
      'Common Values'], tablefmt='pretty', stralign='left'))

# Making another table summarising the parameters which differ from one experiment to another
rows = np.array([np.array(list(x.values()), dtype=object)[
                multi_val_cols] for x in all_params.values()])
headers = np.array(list(list(all_params.values())[0].keys()), dtype=object)[
    multi_val_cols]
for i in range(len(rows)):
    row = [[x] for x in rows[i]]
    check_line_length(row)
    rows[i] = [x[0] for x in row]
    
multi_val_df = pd.DataFrame(rows, columns=headers, index=all_params.keys())
if args.sort_by in headers:
    multi_val_df = multi_val_df.sort_values(args.sort_by, ascending=False)
print(tabulate.tabulate(multi_val_df, headers='keys',
...
if args.save_to is not None:
    ...
    if extension == '.csv':
        ...
        logging.error(
            f'Could not determine file type for extension {extension}')