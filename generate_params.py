    ...
    for param_name, value_list in zip(args.param_name, args.values_list):
        ...
        for param_file in param_list:
            ...
            for val in value_list:
                ...
                p_split = val.split(',')
                tmp = params_out
                keys_list = param_name.split('.')
                for p in keys_list[:-1]:
                    tmp = tmp[p]
                ...
                if len(p_split) > 1:
                    if not p_split[-1]:
                        del p_split[-1]
                    try:
                        tmp[keys_list[-1]] = [int(e) for e in p_split]
                    except ValueError:
                        try:
                            tmp[keys_list[-1]] = [float(e) for e in p_split]
                        except ValueError:
                            tmp[keys_list[-1]] = p_split
                else:
                    try:
                        tmp[keys_list[-1]] = int(p_split[0])
                    except ValueError:
                        try:
                            tmp[keys_list[-1]] = float(p_split[0])
                        except ValueError:
                            tmp[keys_list[-1]] = p_split[0]
                ...
                if isinstance(tmp[keys_list[-1]], list):
                    if subt is not None:
                        if not isinstance(tmp[keys_list[-1]][0], subt):
                            warn = True
                    subt = type(tmp[keys_list[-1]][0])

                if t is not None:
                    if not isinstance(tmp[keys_list[-1]], t):
                        warn = True
                t = type(tmp[keys_list[-1]])

                if warn:
                    logging.warning(f'WARNING! Check type consistency for {param_name}')
